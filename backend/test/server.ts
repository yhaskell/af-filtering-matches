    ...
    describe('POST /api/matches', () => {
        it('returns 400 when query is empty', async () => {
            ...
            assert.deepEqual(
                ...
                {
                    "status": 400,
                    "message": "Search query invalid. compatibility_score should be a number range"
                }
            )
        })
        ...
        it('returns 400 when query is incorrect', async () => {
            ...
            assert.deepEqual(
                ...
                {
                    "status": 400,
                    "message": "Search query invalid. compatibility_score.from should be a number"
                }
            )
        })
        ...
        describe('should filter by', () => {
            it('compatibility_score', async () => {
                const query = { ...defaultQuery, compatibility_score: {
                    from: 50,
                    to: 70
                }}
            ...
            it('height', async () => {
                const query = { ...defaultQuery, height: {
                    from: 160,
                    to: 190
                }}
                ...
                assert.equal(res.body.status, 200)
                assert.equal(res.body.payload.length, 9)
            })

            it('age', async () => {
                const query = { ...defaultQuery, age: {
                    from: 25,
                    to: 40
                }}
                ...
                assert.equal(res.body.status, 200)
                assert.equal(res.body.payload.length, 10)
            })
        })
    })
})
